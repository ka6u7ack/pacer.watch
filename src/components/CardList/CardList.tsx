...
const buttonList: { type: CardType; label: string }[] = [
  { type: 'pace', label: 'PACE( 配速 )' },
  { type: 'cadence', label: 'CADENCE( 步频 )' },
  { type: 'lap', label: 'LAP( 圈速 )' },
  // { type: 'log', label: 'LOG' },
  // { type: 'bmi', label: 'BMI( 体重管理 )' },
]

function CardList(props: CardListProps) {
  const { className } = props

  const [showCardList, setShowCardList] = useState<CardType[]>(['pace', 'cadence', 'lap'])
